...
\usepackage{graphicx}
\usepackage{amsmath,amsfonts,amssymb,amsthm,siunitx,bm,chemfig}
...
\section{ESR}
In the first part of the experiment, we performed an ESR study on a sample of 2,2-diphenyl-1-picrylhydrazyl (DPPH), which is a paramagnetic organic molecule with a single unpaired $e^-$. The orbital angular momentum of this electron is essentially zero,    % find reference
...
The basic principles of NMR are essentially the same as those of ESR. Certain atomic nuclei can have a non-zero overall nuclear spin $\mathbf{S}$ (such as ${}^1$H and ${}^{19}$F, which have an odd number of nucleons), and this gives rise to a nuclear magnetic moment $\bm{\mu}_N$. When placed in a magnetic field of strength $B_0$, whether external or due to other close-by magnetic nuclei and/or orbiting electrons, the nuclear spin can adopt $2S + 1$ possible orientations, each corresponding to a different energy level
\[
...
All of our samples had $S = \tfrac{1}{2}$, which meant there were only two possible energy levels, and the condition for resonance was 
...
\subsection{Analysis}
According to \eqref{eqn: NMR resonance condition}, $\left| \nu / B_0 \right|$ should be a constant, equal to $g \mu_n/h$. It follows that $ g = \frac{h}{\mu_n} \left( \frac{\nu}{B_0} \right)$. We performed a straight-line fit to our plots in figure \ref{fig: NMR resonances}, and got
\begin{alignat*}{2}
...
Our measurements of resonant absorption confirmed the linear relationship predicted by equations \eqref{eqn: resonance condition} and \eqref{eqn: NMR resonance condition} between the magnetic field and the resonant frequency. We demonstrated that, indeed, in both ESR and NMR, an energy splitting occurs when the sample is placed in an external magnetic field, that this creates separate energy levels such that the energy difference between them scales linearly with the applied field, and that energy transitions can be induced between these levels by supplying EM energy at the correct frequency. 

We determined the $g$-factors of DPPH, glycerine, polystyrene and PTFE, which agreed reasonably well with the accepted and/or theoretical values, within the limitations of our apparatus, and we also succeeded in detecting the presence of hydrogen in polythene.

An increase in the precision of our numerical findings could have been achieved by taking further repeated measurements, as the statistical errors due to random fluctuations turned out to be quite significant, as indicated by our standard error estimates. Other potential improvements to the experimental procedure would have been to use stronger $\mathbf{B}$ fields, thus increasing the energy difference between the spin states; and by operating at lower temperatures, which would have increased the difference in the number of occupied lower and upper spin states, so that more electrons/nuclei could absorb energy, resulting potentially in a stronger, better-resolved resonance signal, higher sensitivity, and a more favourable signal-to-noise ratio.

\section*{References}
\begin{thebibliography}{9}
\bibitem{ESR review} Lancaster G 1967 Electron paramagnetic resonance (a review) \textit{J. Mat. Sci.} \textbf{2} 489-495
\end{thebibliography}
