...
and hence its magnetic moment is solely due to the spin angular momentum. This makes it particularly suitable for ESR experiments.
 
\subsection{Theory}
In the absence of any external magnetic field (i.e.\ for a free $e^-$), the two possible spin states (``spin up'', and ``spin down'') are degenerate (which means they correspond to the same energy level).

However, energy splitting of the two states can occur when the $e^-$ is placed in an external magnetic field. Indeed, the $e^-$'s spin gives rise to a magnetic moment $\mathbf{\mu}_S$ which will interact with the applied field.
